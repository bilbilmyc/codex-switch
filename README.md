...
3. 没有精确能力信息时，按用户选择的完整模型 ID 创建条目，不按名称拒绝注册，不设置“兼容模式”。已有模型系列的推理档位模板继续保留。

当前选择模型的 `context_window` 和 `max_context_window` 始终写入设定值，`effective_context_window_percent` 为 100；未设置窗口时使用 128,000。新建条目启用文本输入和输出详细程度参数，不再写入保守窗口限制。配置值代表用户设定，不代表服务商承诺支持相应容量；服务商实际调用能力可通过可选的真实请求验证检查。
