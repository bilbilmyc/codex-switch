...
Token 统计首次读取时会扫描 `~/.codex/sessions/` 与 `~/.codex/archived_sessions/` 下的 JSONL 事件，并把已计算的报表按中转站、时间范围和旧版归属时间窗缓存到 `usage.sqlite3`。后续来源文件或归属时间窗未变化时直接读取本地索引；文件变化、缓存内容损坏或删除数据库时会自动重扫并重建。数据库只保存来源文件的路径、大小、修改时间、统计结果，以及旧版记录的 profile UUID 和时间边界；不保存消息正文、原始日志行、完整配置或 API key。损坏记录、超大单行和符号链接会被跳过。
