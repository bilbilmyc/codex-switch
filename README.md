...
- 应用时按完整模型 ID 复用本机能力目录；未知 GPT 数字型号使用带警告的文本兼容配置，无需逐版本增加硬编码。模型目录随配置事务、备份和恢复一并处理。
...
1. 优先保留当前 `model_catalog_json` 中完整 ID 精确匹配的能力条目，包括用户定制的上下文、输入类型和指令。
2. 如果缺少能力信息，读取本机 `~/.codex/models_cache.json` 中精确匹配的条目。不会拿相似型号的参数套用，也不会把只含 ID 的响应当作能力目录。损坏或不可读的可选缓存会被忽略；损坏的当前目录会阻止应用，避免覆盖原配置。
3. 没有精确能力信息时，继续使用已有的 GLM、受支持 DeepSeek 和 Qwen 文本模板。以 `gpt-` 加数字开头的未知型号使用带警告的兼容配置，不再写死某一个 GPT 版本；名称含图像、音频、实时、转录、语音合成、嵌入、审核、视频或搜索标记的型号不会仅凭名称自动注册。

GPT 兼容配置采用 128,000 上下文、文本输入、low/medium/high 推理档位。这些是未验证的兼容参数，不是官方能力声明，也不保证服务商支持。应用后界面会提示“兼容模式”；以后本机缓存提供了该型号的能力信息，再次应用即可替换兼容条目。其他没有能力信息或内置模板的模型仍可写为默认模型，但不会被自动注册到菜单。
...
以后接入新 GPT 文本型号的流程为：**刷新模型 → 选择新型号 → 保存并应用 → 深度验证 → 重启 Codex**。本工具不联网维护官方能力数据库；模型需要新的协议或工具格式时，仍需升级适配代码。深度验证通过也只证明当次检查成功，不保证全部能力或后续请求可用。
