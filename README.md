...
- 应用 GLM、DeepSeek 或 Qwen 文本模型时，自动生成 Codex 所需的模型目录；目录随配置事务、备份和恢复一并处理。
...
当默认模型 ID 属于 `glm-*`、受支持的 `deepseek-*` 或 Qwen `qwen3*` 文本模型时，Codex Switch 会把模型能力写入 `~/.codex/model-catalogs/codex-switch-models.json`，并将 `model_catalog_json` 指向该文件。该目录会保留此前由 Codex Switch 写入的条目，并与 `config.toml`、`auth.json` 同步备份和恢复。Qwen VL、图像、视频模型，以及当前中转端已验证不支持 Agent capabilities 的模型不会自动加入；它们不应被误用为 Codex 文本代理模型。
